...
namespace OpenDentalGraph {
	public partial class DashboardPanelCtrl:UserControl {
		...
		public void SetCellLayout(int rows,int columns,List<DashboardCell> cells) {
			try {
				...
				for(int rowIndex = 0;rowIndex<rows;rowIndex++) {
					for(int columnIndex = 0;columnIndex<columns;columnIndex++) {
						...
						if(cell!=null) {
							//Switch on all dockable controls types here and create dockable controls accordingly.
							switch(cell.CellType) {
								...
								case DashboardCellType.BrokenApptGraph:
									...
									break;
								default:
									//MessageBox.Show("Unsupported CellType: "+cell.CellType.ToString());
									break;
							}							
						}
