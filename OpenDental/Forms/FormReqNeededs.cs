	...
	public class FormReqNeededs:System.Windows.Forms.Form {
      ...
      this.ResumeLayout(false);
		...
		private void FormRequirementsNeeded_Load(object sender, System.EventArgs e) {
			...
			_listSchoolClasses=new List<SchoolClass>();
			_listSchoolCourses=new List<SchoolCourse>();
			for(int i=0;i<SchoolClasses.List.Length;i++) {
				_listSchoolClasses.Add(SchoolClasses.List[i]);
			...
			for(int i=0;i<SchoolCourses.List.Length;i++) {
				_listSchoolCourses.Add(SchoolCourses.List[i]);
				comboCourseFrom.Items.Add(SchoolCourses.List[i].Descript);
				comboCourseTo.Items.Add(SchoolCourses.List[i].Descript);
			}
		...
		private void butAdd_Click(object sender, System.EventArgs e) {
			...
			FormR.ReqCur.SchoolClassNum=SchoolClasses.List[comboClassFrom.SelectedIndex].SchoolClassNum;
			FormR.ReqCur.SchoolCourseNum=SchoolCourses.List[comboCourseFrom.SelectedIndex].SchoolCourseNum;
			FormR.IsNew=true;
			FormR.ShowDialog();
		...
		private void gridMain_CellDoubleClick(object sender,ODGridClickEventArgs e) {
			...
			if(FormR.DialogResult==DialogResult.OK) {
				if(FormR.ReqCur==null) {
					...
					int index=_listReqsAll.IndexOf(FormR.ReqCur);
					_listReqsAll[index]=FormR.ReqCur;
				}
				FillGrid();
			}
			if(FormR.DialogResult==DialogResult.OK) {
				RemoveReqFromAllList(FormR.ReqCur);
				FillGrid();
			}
		}
