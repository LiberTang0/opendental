...
namespace OpenDental{
    ...
    "eb.";
		...
		private void FillFields() {
			...
			List<Def> listPayTypes=DefC.Short[(int)DefCat.PaymentTypes].ToList();
			listPayTypes.ForEach(x => comboPaymentType.Items.Add(x.ItemName));
			comboPaymentType.SelectedIndex=listPayTypes.FindIndex(x => x.DefNum.ToString()==payTypeDefNum);
		...
		private void butOK_Click(object sender,System.EventArgs e) {
			...
			else if(checkEnabled.Checked && (textPath.Text.Trim().Length==0 || !File.Exists(textPath.Text.Trim()))) {
